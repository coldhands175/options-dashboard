// Test script for Claude Sonnet 4 options trading document processing
// Usage: node test-claude.js path/to/your/document.pdf

import fs from 'fs';
import path from 'path';

async function testClaudePrompt(pdfFilePath) {
...
Please analyze the provided PDF document now and return the structured JSON response.`;
  ...
  const requestBody = {
    model: "claude-3-5-sonnet-20241022", // Using 3.5 Sonnet as fallback if 4.0 not available
...
if (process.argv.length < 3) {
  console.error('Usage: node test-claude.js <path-to-pdf-file>');
  console.error('Example: node test-claude.js ~/Downloads/statement.pdf');
  console.error('');
