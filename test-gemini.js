// Test script for the enhanced Gemini prompt
// Usage: node test-gemini.js path/to/your/document.pdf

const fs = require('fs');
const path = require('path');

async function testGeminiPrompt(pdfFilePath) {
  ...
  // Your enhanced prompt (same as in convex/documentProcessing.ts)
...
Process the document now:`;
  ...
  try {
    console.log('Processing PDF with enhanced Gemini prompt...');
    ...
    try {
      ...
      parsed.trades.forEach((trade, index) => {
        ...
        console.log(`  Symbol: ${trade.symbol} ${trade.expirationDate} ${trade.strikePrice}${trade.contractType}`);
        console.log(`  Quantity: ${trade.quantity}`);
        console.log(`  Premium: $${trade.premium}`);
        console.log(`  Book Cost: $${trade.bookCost}`);
        console.log(`  Status: ${trade.status}`);
        console.log(`  Confidence: ${(trade.extractionConfidence * 100).toFixed(1)}%`);
        if (trade.notes) {
...
if (process.argv.length < 3) {
  console.error('Usage: node test-gemini.js <path-to-pdf-file>');
  console.error('Example: node test-gemini.js ~/Downloads/statement.pdf');
  process.exit(1);
}

const pdfFilePath = process.argv[2];
testGeminiPrompt(pdfFilePath);
